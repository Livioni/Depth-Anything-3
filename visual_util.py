...
def predictions_to_ply(
    ...
    """
    Converts VGGT predictions to a 3D point cloud and saves it as a PLY file.
    ...
    Args:
        ...
        output_filenae (str): Output PLY file name (default: "point_cloud.ply")

    Returns:
        None: Saves the 3D point cloud as a PLY file at the specified path.
    """
    # Validation and extraction of the required predictions
...
def depth_evaluation(predicted_depth_original, ground_truth_depth_original, max_depth=80, custom_mask=None, post_clip_min=None, post_clip_max=None, pre_clip_min=None, pre_clip_max=None,
    ...
    if predicted_depth_original.shape[1] > 1:
        predicted_depth_original = predicted_depth_original.squeeze()
        ground_truth_depth_original = ground_truth_depth_original.squeeze()
    else:
        predicted_depth_original = predicted_depth_original.squeeze(0)
        ground_truth_depth_original = ground_truth_depth_original.squeeze(0).squeeze(-1)
    ...
    if custom_mask is not None:
        mask = mask & (custom_mask.to(predicted_depth_original.device) > 0)
    ...
    mask = mask.to(depth_error_parity_map.device)
