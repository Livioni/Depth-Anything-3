...
class Droid(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='<here is your dataset location>',  # for example /mnt/local/lihao/phs/datasets/droid/clean_datasets
                 dset='',
                 use_cache=False,
                 use_augs=False,
                 top_k=256,
        ...
        if self.use_cache:
            dataset_location = '<here is your annotation path>'  # for example /mnt/lihao/phs_datasets/annotations/droid_annotations
            ...
            for full_index, seq in enumerate(self.sequences):
                ...
                if seq == "/mnt/local/lihao/phs/datasets/droid/clean_datasets/Tue_May__9_04:20:30_2023/":
                    continue
                ...
                for index, sub_seq in enumerate(os.listdir(seq)):
                    ...
                    # 这个是omniworld depth
                    depth_path = os.path.join(seq, sub_seq, 'raw_depth')
                    
                    #这是ma pose
                    # extrinsic_path = glob.glob(os.path.join(seq, sub_seq, 'poses_ma','poses.npy'))[0]
                    # 这是gt pose
                    # extrinsic_path = glob.glob(os.path.join(seq, sub_seq, 'poses_ma','poses_depth_ba.npy'))[0]
                    # 这是原始pose
...
if __name__ == "__main__":
    ...
    dataset_location = '<here is your dataset location>'  # Change this to the correct path
    dset = ''
    use_augs = False
    num_views = 6
    ...
    def visualize_scene(idx):
        ...
        # return viz.show()
        viz.save_glb('droid-800-oripose-omnidepth.glb')
        return 
    ...
    visualize_scene((800,0,num_views))