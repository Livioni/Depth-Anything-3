...
def _load_cam(path):
    """Load camera parameters from .npz or .safetensor."""
    if path.endswith('.safetensor') or path.endswith('.safetensors'):
        d = load_safetensors(path)
        # mapfree uses 'pose' and 'intrinsic'
        pose = np.array(d['pose'], dtype=np.float32)
        intrinsics = np.array(d.get('intrinsic', d.get('intrinsics')), dtype=np.float32)
    else:
        d = np.load(path)
        pose = np.array(d['pose'], dtype=np.float32)
        intrinsics = np.array(d.get('intrinsic', d.get('intrinsics')), dtype=np.float32)
...
class MapFree(BaseStereoViewDataset):
    def __init__(self,
                 dataset_location='<here is your dataset location>',  # for example /mnt/local/lihao/phs_datasets/mapfree
                 dset='',
                 use_cache=False,
                 use_augs=False,
                 top_k=256,
        ...
        if self.use_cache:
            anno_root = '<here is your annotation path>'  # for example /mnt/local/lihao/phs_datasets/annotations/mapfree_annotations
            ...
            with open(os.path.join(anno_root, dset, 'sky_mask_paths.json'), 'r') as f:
                self.all_sky_mask_paths = json.load(f)
            ...
            depth_mask_paths_file = os.path.join(anno_root, dset, 'depth_mask_paths.json')
            if os.path.isfile(depth_mask_paths_file):
                with open(depth_mask_paths_file, 'r') as f:
                    depth_mask_paths_raw = json.load(f)
            ...
            for seq in self.sequences:
                if self.verbose:
                    print('seq', seq)
                for sub_seq in sorted(os.listdir(seq)):
                    sub_path = os.path.join(seq, sub_seq)
                    if not os.path.isdir(sub_path):
                        continue
                    rgb_dir = os.path.join(sub_path, 'rgb')
                    depth_dir = os.path.join(sub_path, 'depth')
                    cam_dir = os.path.join(sub_path, 'cam')
                    sky_dir = os.path.join(sub_path, 'sky_mask')
                    depth_mask_dir = os.path.join(sub_path, 'depth_mask')

                    rgb_files = sorted(glob.glob(os.path.join(rgb_dir, '*.jpg')))
                    depth_files = sorted(glob.glob(os.path.join(depth_dir, '*.npy')))
                    sky_files = sorted(glob.glob(os.path.join(sky_dir, '*.jpg')))
                    depth_mask_files = sorted(glob.glob(os.path.join(depth_mask_dir, '*.png'))) \
                        if os.path.isdir(depth_mask_dir) else []
                    cam_files = sorted(glob.glob(os.path.join(cam_dir, '*.safetensor'))) \
                        or sorted(glob.glob(os.path.join(cam_dir, '*.safetensors'))) \
                        or sorted(glob.glob(os.path.join(cam_dir, '*.npz')))

                    num_frames = len(rgb_files)
                    if num_frames < 24:
                        print(f'skipping {sub_path}, too few images ({num_frames})')
                        continue
                    if not (num_frames == len(depth_files) == len(cam_files) == len(sky_files)):
                        print(f'skipping {sub_path}: count mismatch '
                              f'rgb={num_frames} depth={len(depth_files)} cam={len(cam_files)} sky={len(sky_files)}')
                        continue
                    ...
                    self.all_sky_mask_paths.extend(sky_files)
                    if len(depth_mask_files) == num_frames:
                        self.all_depth_mask_paths.extend(depth_mask_files)
                    else:
                        self.all_depth_mask_paths.extend([None] * num_frames)

                    extrinsics_seq = []
                    for anno in cam_files:
                        pose, intr = _load_cam(anno)
            ...
            # self._save_paths_to_json(self.all_sky_mask_paths, os.path.join(anno_root, 'sky_mask_paths.json'))
    ...
    def _get_views(self, index, num, resolution, rng):
        ...
        sky_paths = [self.all_sky_mask_paths[i] for i in full_idx]
        ...
        views = []
        for impath, depthpath, skypath, depth_mask_path, camera_pose, intrinsics in zip(
                rgb_paths, depth_paths, sky_paths, depth_mask_paths, camera_pose_list, intrinsics_list):
            rgb_image = Image.open(impath).convert('RGB')
            depthmap = np.load(depthpath).astype(np.float32)
            sky_mask = cv2.imread(skypath, cv2.IMREAD_GRAYSCALE) == 0
            depthmap[~sky_mask] = 0
            depthmap[depthmap > self.z_far] = 0.0
...
if __name__ == "__main__":
    ...
    def visualize_scene(idx, out='mapfree_scene.glb'):
        views = dataset[idx]
        viz = SceneViz()
