...
class Re10K(BaseStereoViewDataset):
    def __init__(self,
        ...
        if self.use_cache:
            dataset_location = 'annotations/re10k_annotations'
            ...
            # 保存为 JSON 文件
            os.makedirs(f'annotations/re10k_annotations/{self.dset}', exist_ok=True)
            self._save_paths_to_json(self.all_rgb_paths, f'annotations/re10k_annotations/{self.dset}/rgb_paths.json')
            joblib.dump(self.rank, f'annotations/re10k_annotations/{self.dset}/rankings.joblib')
            joblib.dump(self.all_extrinsic, f'annotations/re10k_annotations/{self.dset}/extrinsics.joblib')
            joblib.dump(self.all_intrinsic, f'annotations/re10k_annotations/{self.dset}/intrinsics.joblib')
            joblib.dump(self.all_mask_paths, f'annotations/re10k_annotations/{self.dset}/mask_gts.joblib')
            joblib.dump(self.all_mask_index, f'annotations/re10k_annotations/{self.dset}/mask_index.joblib')
            print('found %d frames in %s (dset=%s)' % (len(self.full_idxs), dataset_location, dset))
...
if __name__ == "__main__":
    ...
    dataset = Re10K(
        dataset_location="datasets/processed_re10k",
        mask_gt_location="datasets/processed_re10k/sam2_results",
        dset = '',
        use_cache = False,
        top_k= 50,
        quick=False,
